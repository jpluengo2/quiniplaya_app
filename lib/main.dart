...
class _MainScreenState extends State<MainScreen> {
  // === TAMAÑOS MAESTROS ===
  // El ancho de las cajas grandes (Tablas) sigue fijo para no romper su perfecto diseño.
  final double anchoFijoCajasGrandes = kIsWeb ? 560.0 : 720.0;  
  final double altoFijo1 = kIsWeb ? 270.0 : 320.0; 
  final double altoFijo2 = kIsWeb ? 470.0 : 590.0; 
  ...
  Widget build(BuildContext context) {
    ...
    // === SOLUCIÓN MAESTRA: CÁLCULO DEL ANCHO DINÁMICO DE LA PANTALLA ===
    double screenWidth = MediaQuery.of(context).size.width;
    double screenHeight = MediaQuery.of(context).size.height;
    double availableHeight = screenHeight - 10.0;
    
    // Calculamos qué escala aplicará el FittedBox a los recuadros pequeños
    double scalePequenas = availableHeight / altoFijo1;
    // Forzamos el ancho virtual para que, al aplicarle la escala, coincida exactamente con el ancho de la pantalla (menos los 30px del padding horizontal del ScrollView)
    double anchoDinamicoPequenas = kIsWeb ? 560.0 : ((screenWidth - 30.0) / scalePequenas);

    // Asignamos el ancho dinámico solo a las pantallas de Presentación y Datos
    Widget presentacion = PresentationBox(width: anchoDinamicoPequenas, height: altoFijo1);
    Widget datosBase = GeneralDataBox(width: anchoDinamicoPequenas, height: altoFijo1, datos: jsonData!['datosGenerales']);
    Widget datosFallos = esFallos ? FallosDataBox(width: anchoDinamicoPequenas, height: altoFijo1, datos: jsonData!['datosFallos']) : const SizedBox.shrink();

    // Las tablas complejas mantienen su ancho fijo gigante para no deformarse
    Widget pronosticos = PronosticosBox(width: anchoFijoCajasGrandes, height: altoFijo2, partidos: List<String>.from(jsonData!['partidos'] ?? []), pronosticosBase: List<String>.from(jsonData!['pronosticosBase'] ?? []), pronosticosFallos: List<String>.from(jsonData!['pronosticosFallos'] ?? []), resultados: resultados, onResultadoChanged: _actualizarResultado, recuentoGlobal: recuentoGlobal);
    Widget boletoBase = BoletoBox(width: anchoFijoCajasGrandes, height: altoFijo2, titulo: 'Carrusel de Boletos Base', apuestas: List<String>.from(jsonData!['apuestasBase'] ?? []), resultados: resultados);
    Widget boletoFallos = esFallos ? BoletoBox(width: anchoFijoCajasGrandes, height: altoFijo2, titulo: 'Carrusel Boletos con Fallo', apuestas: List<String>.from(jsonData!['apuestasFallos'] ?? []), resultados: resultados) : const SizedBox.shrink();
  ...
  @override Widget build(BuildContext context) {
    return GridItemContainer(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text('LA QUINIPLAYA MILLONARIA', textAlign: TextAlign.center, style: TextStyle(fontSize: kIsWeb ? 20 : 22, color: const Color.fromRGBO(207, 7, 7, 0.938), fontWeight: FontWeight.bold)), 
        const SizedBox(height: 20), 
        ClipRRect(borderRadius: BorderRadius.circular(8.0), child: Image.asset('assets/dadoqpr.png', height: 75, fit: BoxFit.cover)), 
        const SizedBox(height: 20), 
        Text('Sistema Combinatorio de Alto Rendimiento para Apuestas Deportivas', textAlign: TextAlign.center, style: TextStyle(fontSize: kIsWeb ? 12 : 14, color: const Color(0xFF080868), fontWeight: FontWeight.bold))
      ])
    );
  }
}
...
class GeneralDataBox extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return GridItemContainer(
      width: width, height: height, paddingVertical: kIsWeb ? 2.0 : 8.0, 
      child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text('Datos Generales Jornada en Curso', textAlign: TextAlign.center, style: TextStyle(fontSize: kIsWeb ? 16 : 18, color: const Color.fromRGBO(207, 7, 7, 0.938), fontWeight: FontWeight.bold)), 
  ...
  TableRow _buildDataRow(String label, String value) { 
    return TableRow(children: [Padding(padding: EdgeInsets.symmetric(vertical: kIsWeb ? 2.0 : 2.0, horizontal: 8.0), child: Text(label, textAlign: TextAlign.right, style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: kIsWeb ? 10 : 12))), Padding(padding: EdgeInsets.symmetric(vertical: kIsWeb ? 2.0 : 2.0, horizontal: 8.0), child: FittedBox(alignment: Alignment.centerLeft, fit: BoxFit.scaleDown, child: Text(value, textAlign: TextAlign.left, style: TextStyle(color: const Color(0xFF080868), fontWeight: FontWeight.bold, fontSize: kIsWeb ? 10 : 12))))]); 
  }
}
...
class FallosDataBox extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return GridItemContainer(
      width: width, height: height, paddingVertical: kIsWeb ? 2.0 : 8.0, 
      child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text('Datos Adicionales Sistema de Fallos', textAlign: TextAlign.center, style: TextStyle(fontSize: kIsWeb ? 16 : 18, color: const Color.fromRGBO(207, 7, 7, 0.938), fontWeight: FontWeight.bold)), 
...
class PronosticosBox extends StatelessWidget {
    ...
    return GridItemContainer(
      ...
      Expanded(child: Table(columnWidths: const { 0: FlexColumnWidth(4.5), 1: FlexColumnWidth(2.0), 2: FlexColumnWidth(2.0), 3: FlexColumnWidth(2.5) }, defaultVerticalAlignment: TableCellVerticalAlignment.middle, 
        children: [
          ...
          const TableRow(children: [SizedBox(height: 4.0), SizedBox(height: 4.0), SizedBox(height: 4.0), SizedBox(height: 4.0)]),
        ]))])
    );
  }
  ...
  TableRow _buildPartidoRow(int index, String partido) {
    ...
    return TableRow(children: [
      Padding(
        padding: EdgeInsets.symmetric(vertical: kIsWeb ? 3.5 : 3.0).copyWith(right: 25.0), 
        child: FittedBox(
          alignment: Alignment.centerLeft,
          fit: BoxFit.scaleDown,
          child: Text(nombrePartido, style: TextStyle(color: const Color(0xFF080868), fontWeight: FontWeight.bold, fontSize: kIsWeb ? 13 : 21))
        )
      ), 
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 10.0), child: Container(height: kIsWeb ? 24 : 36, padding: const EdgeInsets.symmetric(horizontal: 4.0), decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(6.0), border: Border.all(color: Colors.black, width: 1.0)), child: DropdownButtonHideUnderline(child: DropdownButton<String>(value: resultado, isExpanded: true, isDense: true, icon: Icon(Icons.keyboard_arrow_down, color: Colors.black, size: kIsWeb ? 18 : 28), dropdownColor: Colors.white, borderRadius: BorderRadius.circular(12.0), alignment: Alignment.center, style: TextStyle(color: const Color(0xFF080868), fontWeight: FontWeight.bold, fontSize: kIsWeb ? 12 : 21), items: const [DropdownMenuItem(value: '0', alignment: Alignment.center, child: Text(' ')), DropdownMenuItem(value: '1', alignment: Alignment.center, child: Text('1')), DropdownMenuItem(value: 'X', alignment: Alignment.center, child: Text('X')), DropdownMenuItem(value: '2', alignment: Alignment.center, child: Text('2'))], onChanged: (val) { if (val != null) onResultadoChanged(index, val); })))), 
...
class _BoletoBoxState extends State<BoletoBox> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return GridItemContainer(
      width: widget.width, height: widget.height, paddingHorizontal: kIsWeb ? 35.0 : 5.0, paddingVertical: kIsWeb ? 2.0 : 4.0,
      child: Column(children: [Row(children: [GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => currentTicket = (currentTicket - 1 + totalTickets) % totalTickets), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 15.0, vertical: 2.0), child: Text('<<', style: TextStyle(fontSize: kIsWeb ? 28 : 38, color: const Color(0xFF080868), fontWeight: FontWeight.w900, letterSpacing: -2)))), const Spacer(), Text(widget.titulo, style: TextStyle(fontSize: kIsWeb ? 22 : 28, color: const Color.fromRGBO(207, 7, 7, 0.938), fontWeight: FontWeight.bold)), const Spacer(), GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => setState(() => currentTicket = (currentTicket + 1) % totalTickets), child: Padding(padding: const EdgeInsets.symmetric(horizontal: 15.0, vertical: 2.0), child: Text('>>', style: TextStyle(fontSize: kIsWeb ? 28 : 38, color: const Color(0xFF080868), fontWeight: FontWeight.w900, letterSpacing: -2))))]), const SizedBox(height: 2), Expanded(child: Table(columnWidths: const { 0: FlexColumnWidth(1.2), 1: FlexColumnWidth(1.0), 2: FlexColumnWidth(1.0), 3: FlexColumnWidth(1.0), 4: FlexColumnWidth(1.0), 5: FlexColumnWidth(1.0), 6: FlexColumnWidth(1.0), 7: FlexColumnWidth(1.0), 8: FlexColumnWidth(1.0) }, defaultVerticalAlignment: TableCellVerticalAlignment.middle, children: [_buildTopHeaderRow(apuestasEnEsteBoleto, aciertosBoletoActual), ...List.generate(14, (index) => _buildBetRow(index + 1, apuestasEnEsteBoleto, startIndex)), _buildBottomHeaderRow(apuestasEnEsteBoleto, startIndex)]))])
    );
  }

  TableRow _buildTopHeaderRow(int numApuestas, List<int> aciertos) { return TableRow(children: [_buildHeaderText('B${currentTicket + 1}'), ...List.generate(8, (index) { if (index >= numApuestas) return _buildHeaderText(''); int count = aciertos[index]; return Container(height: kIsWeb ? 22 : 32, alignment: Alignment.center, color: count > 9 ? Colors.yellow : Colors.transparent, child: Text(count.toString(), style: TextStyle(color: const Color(0xFF080868), fontWeight: FontWeight.bold, fontSize: kIsWeb ? 12 : 21))); })]); }
  ...
  Widget _buildHeaderText(String text) { return Container(height: kIsWeb ? 22 : 32, alignment: Alignment.center, child: Text(text, style: TextStyle(color: const Color(0xFF080868), fontWeight: FontWeight.bold, fontSize: kIsWeb ? 12 : 21))); }
  ...
  Widget _buildBetCell({required bool isOdd, required String markedSymbol, required String resultadoPartido}) { 
    return TableCell(
      ...
      child: Container(
        ...
        padding: EdgeInsets.symmetric(horizontal: 6.0, vertical: kIsWeb ? 2.5 : 2.5), 
  ...
  Widget _buildSmallSquare(String text, {required bool isMarked, required bool isOddColumn, required String resultado}) {
    ...
    return Expanded(
      child: Container(
        ...
        child: FittedBox(fit: BoxFit.scaleDown, child: Text(text, textAlign: TextAlign.center, style: TextStyle(color: textColor, fontWeight: isMarked ? FontWeight.bold : FontWeight.normal, fontSize: kIsWeb ? 11 : 21)))
      )
    );
  }
}